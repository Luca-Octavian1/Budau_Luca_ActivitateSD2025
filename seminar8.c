...
float calculeazaPretTotal(Nod* arbore) {
	if (arbore) {
		float totSt = calculeazaPretTotal(arbore->st);
		float totDr = calculeazaPretTotal(arbore->dr);
		return totDr + totSt + arbore->info.pret;
	}
	else return 0;
}
...
int main() {
	...
	printf("\n----------------------\nSuma tuturor masinilor: %.2f", calculeazaPretTotal(radacina));
